#!/bin/bash
# ============================================================
#  Zoho Robotics — Developer local system Setup Script
#  Runs once on a fresh machine. Safe to re-run.
#
#  Versions are read from versions.env in the same directory.
#  To upgrade a tool, update versions.env — not this script.
# ============================================================
...
if [[ ! -f "$VERSIONS_FILE" ]]; then
  echo ""
  echo -e "${RED}✖  versions.env not found at: $VERSIONS_FILE${RESET}"
  echo ""
  echo "  This script requires versions.env to be in the same"
  echo "  directory as setup_intern_laptop.sh."
  echo ""
  echo "  Ask your mentor for the versions.env file."
  echo ""
  exit 1
fi
...
for var in "${REQUIRED_VARS[@]}"; do
  if [[ -z "${!var}" ]]; then
    echo -e "${RED}✖  versions.env is missing required variable: $var${RESET}"
    MISSING_VARS=true
  fi
done
if [[ "$MISSING_VARS" == "true" ]]; then
  echo ""
  echo "  versions.env is incomplete. Ask your mentor for the correct file."
  exit 1
fi
...
echo ""
echo -e "${BOLD}╔══════════════════════════════════════════════════════════╗${RESET}"
echo -e "${BOLD}║   Zoho Robotics — Developer PC/laptop Setup                ║${RESET}"
echo -e "${BOLD}║   Reads tool versions from versions.env                  ║${RESET}"
echo -e "${BOLD}╚══════════════════════════════════════════════════════════╝${RESET}"
echo ""
echo "  Configuration loaded from: $VERSIONS_FILE"
echo ""
...
[[ "$(git config --global init.defaultBranch 2>/dev/null)" == "main" ]] && \
  pass "init.defaultBranch already set to main" || \
  { git config --global init.defaultBranch main && pass "init.defaultBranch set to main"; }
...
if command -v node &>/dev/null; then
  ...
  elif [[ "$INSTALLED_MAJOR" -lt "$NODE_MAJOR_VERSION" ]]; then
...
if [[ "$NEEDS_NODE" == "true" ]]; then
  # Fix broken Chrome GPG keys if present — they block apt updates
  if sudo apt-get update 2>&1 | grep -q "FD533C07C264648F\|signatures couldn't be verified"; then
    info "Detected broken Chrome GPG keys — fixing before Node.js install..."
    ...
    pass "Chrome GPG keys fixed"
  fi

  # Remove old NodeSource repo and Node.js
  sudo apt-get remove -y nodejs npm 2>/dev/null || true
  sudo rm -f /etc/apt/sources.list.d/nodesource.list 2>/dev/null || true

  # Install target Node.js version via NodeSource
...
if [[ -d "$ROS2_PATH" ]]; then
  ...
  if [[ -n "$INSTALLED_DISTROS" ]]; then
    ...
    warn "If your team is on a different distro, update ROS2_DISTRO in versions.env"
  else
...
[[ "$BRANCH_CHECK" == "main" ]]  && pass "init.defaultBranch:  main"               || { fail "init.defaultBranch not set"; ALL_GOOD=false; }
