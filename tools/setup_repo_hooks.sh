...
#
#  Versions are read from tools/versions.env in the repository.
...
# ── Load versions.env from repository's tools/ directory ─────
...
echo ""
echo -e "  ${CYAN}Tool versions from versions.env:${RESET}"
...
# Node.js — check against NODE_MAJOR_VERSION from versions.env
if ! command -v node &>/dev/null; then
  fail "Node.js is not installed"
  info "Run setup_intern_laptop.sh first"
  PREREQS_OK=false
else
  ...
  if [[ "$INSTALLED_MAJOR" -ge "$NODE_MAJOR_VERSION" ]]; then
    ...
    info "Run setup_intern_laptop.sh to upgrade"
    PREREQS_OK=false
  fi
fi

# pre-commit — check against PRECOMMIT_MIN_VERSION from versions.env
...
if [[ "$CURRENT_BRANCH" == "develop" ]]; then
  pass "Already on develop branch"
elif git show-ref --verify --quiet refs/remotes/origin/develop; then
  info "Switching to develop..."
  git switch develop 2>/dev/null || git checkout develop
  pass "Switched to develop"
else
  warn "develop branch not found — staying on $CURRENT_BRANCH"
  warn "Ask your Engineering Lead whether develop has been created"
fi
...
if [[ -d "node_modules/husky" ]] && [[ -d "node_modules/@commitlint" ]]; then
  # Verify installed commitlint version matches versions.env if specified
...
info "Running npx husky install..."
npx husky install
...
TEST1_OUTPUT=$(git commit --allow-empty -m "fix stuff" 2>&1 || true)
if echo "$TEST1_OUTPUT" | grep -qiE "problems|error|rejected|subject|scope|type"; then
  ...
  git config core.hooksPath .husky
  # Retry test
  TEST1_RETRY=$(git commit --allow-empty -m "fix stuff" 2>&1 || true)
...
TEST2_OUTPUT=$(git commit --allow-empty -m "feat(arm): test automation hooks setup" 2>&1)
...
if [[ "$HOOKS_PATH_FINAL" == ".husky" && "$HOOKS_OK" == "true" ]]; then
  ...
  echo ""
  echo "  versions.env: $VERSIONS_FILE"
  echo ""
